...
def eval():
    ...
    def find_var(filename,mf = False):
        if mf:
            with xr.open_dataset(filename[0], engine='h5netcdf') as ds:
                var2 = ds['nitrate'].isel(time_counter=0)
                for idx, row in group.iterrows():
                    if row.k_above == 'NaN':
                        continue
                    ab2 = var2.isel(deptht=slice(row.k_above, row.k_above + 2),y=row.j,x=row.i).values
                    N_model[idx] = interp_depth(ab2[0], ab2[1],row.z_above, row.z_bellow,row.Depth)
            with xr.open_dataset(filename[1], engine='h5netcdf') as ds:
                var = ds['dissolved_oxygen'].isel(time_counter=0)
                for idx, row in group.iterrows():
                    if row.k_above == 'NaN':
                        continue
                    ab = var.isel(deptht=slice(row.k_above, row.k_above + 2),y=row.j,x=row.i).values
                    DO_model[idx] = interp_depth(ab[0], ab[1],row.z_above, row.z_bellow,row.Depth)
        else:
    ...
    runs = ['SSBase','SHEM18','tuning/diat_pref','tuning/exc_hbac','tuning/exc_hbac_2','tuning/growth_flag','tuning/growth_flag_2','tuning/mort_hbac','tuning/pred_flag','tuning/remin','tuning/remin2','tuning/predmine','tuning/mort_hbac_2','tuning/predmine_z2']
    names = ['SSBase','SHEM18','diat_pref','exc_hbac','exc_hbac_2','growth_flag','growth_flag_2','mort_hbac','pred_flag','remin','remin2','predmine','mort_hbac_2','predmine_z2']
    for i,name in enumerate(names):
        print(f'Starting: {runs[i]}')
        path = f'/home/jvalenti/scratch/run_SHEM/{runs[i]}/'
        N_model = np.full(len(df), np.nan)
        DO_model = np.full(len(df), np.nan)
        for folder_day, group in df.groupby('folder_day'):
            try:
                if name=='SSBase':
                    fn2 = make_filename(path,row.folder_day,'chem_T')
                    fn = make_filename(path,row.folder_day)
                    N_model, DO_model = find_var([fn,fn2], mf=True)
                    df[name+'_NO3'] = N_model
                    df[name+'_DO'] = DO_model
                else:
                    fn = make_filename(path,row.folder_day)
                    N_model, DO_model = find_var(fn)
                    df[name+'_NO3'] = N_model
                    df[name+'_DO'] = DO_model
        ...
        df[nameno3] = N_model
        df[namedo] = DO_model
    df.to_csv('eval_model_whole.csv', index=False)
