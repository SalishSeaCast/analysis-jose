...
def NO3():
    ...
    for row in df.itertuples(index=False):
        ...
        if row.k_above == 'NaN':
            ...
            ab = fl['nitrate'].isel(time_counter=0,deptht=row.k_above,y=row.j,x=row.i).item()
            be = fl['nitrate'].isel(time_counter=0,deptht=row.k_above+1,y=row.j,x=row.i).item()
    ...
    df.to_excel('NO_model_'+config[0]+'.xlsx')
