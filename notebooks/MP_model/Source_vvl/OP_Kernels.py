...
def Buoyancy(particle, fieldset, time):
    ...
    if particle.status == 1: #Check particle is in the water column   
        ...
        t = fieldset.T[time, deps, particle.lat, particle.lon] #Loading temperature from SSC
        ro = fieldset.R[time, deps, particle.lat, particle.lon] #Loading density sw from SSC
...
def turb_mix(particle,fieldset,time):
    """Vertical mixing"""
    td = 300#fieldset.totaldepth[time, particle.depth, particle.lat, particle.lon]#Total_depth
    seash = 1#fieldset.ssh[time, particle.depth, particle.lat, particle.lon] #SSH(t)


    if particle.status==1:
        #Vertical mixing
        if particle.depth + 0.5 > td+seash: #Only calculate gradient of diffusion for particles deeper than 0.5 otherwise OP will look for particles outside the domain raising an error.
            Kzdz = 2*(fieldset.K_z[time, particle.depth, particle.lat, particle.lon]-fieldset.K_z[time, particle.depth-0.5, particle.lat, particle.lon]) 
            #backwards difference 
        else: 
            Kzdz = 2*(fieldset.K_z[time, particle.depth+0.5, particle.lat, particle.lon]-fieldset.K_z[time, particle.depth, particle.lat, particle.lon]) #forward difference 
        dgrad = Kzdz*particle.dt
        if particle.depth+(0.5*dgrad) > 0 and particle.depth+(0.5*dgrad) < td+seash:
            Kz = fieldset.K_z[time, particle.depth+ 0.5*dgrad, particle.lat, particle.lon] 
        ...
        Swh = 1
        if particle.depth < 5:
            Sbh = fieldset.R[time, 0.51, d_randomy, d_randomx] #Check if particles reach coast at shallow water (SW Density = 0)
            if Sbh==0:
                particle.status = 2 
        else:
            Swh = fieldset.R[time, particle.depth, d_randomy, d_randomx]
            if Swh == 0:
...
def Displacement(particle,fieldset,time):
    ...
    if particle.status == 1:
        #Apply buoyancy
        if particle.depth + dws + particle_ddepth > seash+td:
            particle_ddepth = seash+td-particle.depth #particle reaches the bottom 
        ...
        if dzs + particle_ddepth + particle.depth > seash+td: #crossed bottom in dt (Reflect)
            particle_ddepth = 2*(seash+td) - (dzs + particle_ddepth + 2*particle.depth) #bounce on boundary/ no energy loss
