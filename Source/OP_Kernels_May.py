def Advection(particle, fieldset, time):
    if particle.beached == 0: #Check particle is in the water column
        ...
        if particle.surf == 1: #If particle is floating safe w sampling
            ...
            particle.lon += (u1 + 2*u2 + 2*u3 + u4) / 6. * particle.dt
            particle.lat += (v1 + 2*v2 + 2*v3 + v4) / 6. * particle.dt
            sshn = fieldset.sossheig[time+particle.dt, particle.depth, particle.lat, particle.lon]
...
def turb_mix(particle,fieldset,time):
    ...
    if particle.beached==0:
        ...
        if particle.depth/factor + 0.5 > bath: #Only calculate gradient of diffusion for particles deeper than 0.6 otherwise OP will check for particles outside the domain and remove it.
            Kzdz = 2*(fieldset.vert_eddy_diff[time, particle.depth/factor, particle.lat, particle.lon]-fieldset.vert_eddy_diff[time, particle.depth/factor-0.5, particle.lat, particle.lon]) #backwards difference 
        else: 
            Kzdz = 2*(fieldset.vert_eddy_diff[time, particle.depth/factor+0.5, particle.lat, particle.lon]-fieldset.vert_eddy_diff[time, particle.depth/factor, particle.lat, particle.lon]) #forward difference 
        dgrad = Kzdz*particle.dt/factor
        if particle.depth/factor+0.5*dgrad > 0:
            Kz = fieldset.vert_eddy_diff[time, particle.depth/factor+0.5*dgrad, particle.lat, particle.lon] #Vertical diffusivity SSC  
        else:
            Kz = fieldset.vert_eddy_diff[time, particle.depth/factor, particle.lat, particle.lon] 
        if particle.depth/factor+0.5*dgrad > bath:  
            Kz = fieldset.vert_eddy_diff[time, particle.depth/factor, particle.lat, particle.lon]

        Rr = ParcelsRandom.uniform(-1, 1)
        d_random = sqrt(3*2*Kz*particle.dt) * Rr/factor
        dzs = dgrad + d_random
